...
This guide is currently in **plain assistant** mode for Frostpunk. No in-character persona is active. Responses use the standard assistant voice while honoring all hintforge spoiler-tier, citation, and hint-ladder rules.
...
Say "add a persona" or "switch to a character voice" in a session opened against this guide. The assistant will research two suitable characters from Frostpunk, propose them, and rewrite this file with the two-voice toggle structure from `templates/persona.md` once confirmed.
